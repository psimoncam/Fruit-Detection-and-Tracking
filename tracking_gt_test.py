...
if __name__ == '__main__':
    ...
    # Get GT File
    print("Processing GT file...")
    gt_dict = {}
    gt_file = glob.glob(os.path.join("MOT", os.path.splitext(os.path.split(video_file)[-1])[0], "gt", "*.txt"))[0]
    with open(gt_file) as f:
        lines = f.readlines()
        lines = map(lambda x: x.split(","), lines)
    for line in lines:
        if int(line[0]) not in gt_dict.keys():
            gt_dict[int(line[0])] = []
        gt_dict[int(line[0])].append([int(line[2]), int(line[3]), int(line[4]), int(line[5])])

    # Initialize writer
    fourcc = cv2.VideoWriter_fourcc(*'DIVX')
    writer = cv2.VideoWriter(os.path.split(video_file)[-1].replace(".mp4", "_tracked_th" + th + ".mp4"), fourcc, 20, (2360, 1920), True)
    ...
    deepsort = deepsort_rbc(max_age=opt.maxage, _lambda=opt.lambda_deepsort, max_iou_distance=opt.max_iou_distance, n_init=opt.n_init)
    deepsort_GT = deepsort_rbc(max_age=opt.maxage, _lambda=opt.lambda_deepsort, max_iou_distance=opt.max_iou_distance, n_init=opt.n_init)
    detections = None
    colors = {}
    colors_GT = {}
    pomes_totals = 0
    ...
    separacion = np.zeros((1920,200,3),np.uint8)
    cv2.putText(separacion, "GT/PRED", (30, 960), 0, 5e-3 * 200, (255, 255, 255), 2)
    ...
    while (frame_ID <= last_frame_to_process):
        ...
        if ret and frame_ID in frames_to_process:
            ...
            bboxes_GT = gt_dict[frame_ID]
            out_scores_GT = [1] * len(bboxes_GT)
            
            if opt.enlarge:
                detections = enlarge_bboxs(detections)
                bboxes_GT = enlarge_bboxs(bboxes_GT)
            
            tracker_GT, _ = deepsort_GT.run_deep_sort(frame_GT,out_scores_GT, bboxes_GT)
            try:
                aux = tracker_GT.get_total_tracks()
            except:
                aux = pomes_totals_GT
            pomes_totals_GT = aux
            ...
            try:
                aux2 = tracker.get_total_tracks()
            except:
                aux2 = pomes_totals
            pomes_totals = aux2
            print("Total matches in matching cascade phase:", tracker.total_matches_a)
            print("Total matches in IoU matching phase:", tracker.total_matches_b)
            ...
            for track in tracker_GT.tracks:
                if not track.is_confirmed() or track.time_since_update > 1:
                    continue

                bbox = track.to_tlbr() #Get the corrected/predicted bounding box                                                   
                id_num = str(track.track_id) #Get the ID for the particular track.                                                 
                if id_num not in colors_GT.keys():
                    colors_GT[id_num] = (random.randint(0,255),random.randint(0,255),random.randint(0,255))                                  

                # Overlay bbox from tracker.  
                #print(int(bbox[0]), int(bbox[1]), int(bbox[2]), int(bbox[3]))                      
                cv2.rectangle(frame_GT, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3])), colors_GT[id_num], 3)
                cv2.putText(frame_GT, str(id_num), (int(bbox[0]), int(bbox[1])),0, 5e-3 * 200, (0, 130, 255), 3)
                
                w = int(bbox[2]) - int(bbox[0])
                h = int(bbox[3]) - int(bbox[1])
                
                lines_GT.append(str(frame_ID) + "," + str(id_num) + "," + str(int(bbox[0])) + "," + str(int(bbox[1])) + "," + str(w) + "," + str(h) + "," + str(track.confidence) + "," + str(-1) + "," + str(-1) + "," + str(-1))
                
            bottom_right = 200
            if pomes_totals_GT < 100:
                bottom_right = 200
            elif pomes_totals_GT < 1000:
                bottom_right = 220
            else:
                bottom_right = 240
                
            cv2.rectangle(frame_GT, (5, 1920 - 50, bottom_right, 1920-5), (255,255,255), -1)
            text = "Pomes: {}".format(pomes_totals_GT)
            cv2.putText(frame_GT, text, (10, 1910), 0, 5e-3 * 200, (0, 0, 0), 2)

            frames_concatenated = cv2.hconcat([frame_GT, separacion, frame])

            print("Pomes fins el frame {}: {}\n".format(frame_ID, pomes_totals))
            writer.write(frames_concatenated)
    ...
    dst_folder = "TH" + th
    with open(os.path.join("test_" + dst_folder, os.path.split(video_file)[-1].replace(".mp4", ".txt")), 'w') as test_file:
        test_file.write("\n".join(lines))
        
    with open(os.path.join("GT_" + dst_folder, os.path.split(video_file)[-1].replace(".mp4", ".txt")), 'w') as GT_file:
        GT_file.write("\n".join(lines_GT))
