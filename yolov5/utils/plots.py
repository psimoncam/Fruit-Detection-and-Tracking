...
def plot_results(file='path/to/results.csv', dir=''):
    ...
    for fi, f in enumerate(files):
        try:
            ...
            x = data.values[:, 0]
            for i, j in enumerate([5, 6, 7, 12, 13, 14]):
                y = data.values[:, j]
    ...
    plt.close()
    fig2, ax2 = plt.subplots(2, 4, figsize=(12, 6), tight_layout=True)
    ax2 = ax2.ravel()
    for fi, f in enumerate(files):
        try:
            data = pd.read_csv(f)
            s = [x.strip() for x in data.columns]
            x = data.values[:, 0]
            for i, j in enumerate([1, 2, 3, 4, 8, 9, 10, 11]):
                y = data.values[:, j]
                # y[y == 0] = np.nan  # don't show zero values
                ax2[i].plot(x, y, marker='.', label=f.stem, linewidth=2, markersize=8)
                ax2[i].set_title(s[j], fontsize=12)
                # if j in [8, 9, 10]:  # share train and val loss y axes
                #     ax[i].get_shared_y_axes().join(ax[i], ax[i - 5])
        except Exception as e:
            LOGGER.info(f'Warning: Plotting error for {f}: {e}')
    ax2[1].legend()
    fig2.savefig(save_dir / 'results2.png', dpi=200)
    plt.close()
