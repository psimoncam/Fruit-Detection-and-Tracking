from deepsort_lab.deep_sort.deep_sort import nn_matching
from deepsort_lab.deep_sort.deep_sort.tracker import Tracker 
from deepsort_lab.deep_sort.application_util import preprocessing as prep
from deepsort_lab.deep_sort.deep_sort.detection import Detection
# from deepsort_lab.deep_sort.tools import generate_detections
...
class deepsort_rbc():
	def __init__(self,wt_path=None, max_age=100, _lambda=0, max_iou_distance=0.7, n_init=3):
		#loading this encoder is slow, should be done only once.
		#self.encoder = generate_detections.create_box_encoder("./cosine_metric_learning/weights/apple_9476.pb")		
		if wt_path is not None:
			self.encoder = torch.load(wt_path)			
		else:
			self.encoder = torch.load('./deepsort_lab/ckpts/model640.pt')
		...
		self.tracker= Tracker(self.metric, max_iou_distance=max_iou_distance, max_age=max_age, n_init=n_init, _lambda=_lambda)
