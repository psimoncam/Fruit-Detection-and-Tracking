...
class Tracker:
    ...
    def __init__(self, metric, max_iou_distance=0.7, max_age=100, n_init=3, _lambda=0):
        self.metric = metric
        ...
        self.tracks = []
        self._next_id = 1
        self.confirmed_tracks_id = []
        self.total_tracks = 0
        self.total_matches_a = 0
        self.total_matches_b = 0

    def get_total_tracks(self):
        return len(list(set(self.confirmed_tracks_id)))
    ...
    def update(self, detections):
        ...
        matches, unmatched_tracks, unmatched_detections, num_matches_a, num_matches_b = \
            self._match(detections)

        self.total_matches_a += num_matches_a
        self.total_matches_b += num_matches_b
        ...
        confirmed = [t.track_id for t in self.tracks if t.is_confirmed()]
        #confirmed = [t.track_id for t in self.tracks if not t.is_deleted()]
        self.confirmed_tracks_id.extend(filter(lambda x: x not in self.confirmed_tracks_id, confirmed))
